...
void atm_process_command(ATM *atm, char *command){
  ...
  char* command_regex_string 
    = "^\\s*(begin-session|balance|withdraw|end-session)\\s+";
...
void atm_exec(ATM *atm, char* command, char* full_command){
  ...
  if(!strncmp(command, END, strlen(END))){
    ...
    regex_t begin_regex;
    char* begin_regex_string = "^\\s*begin-session\\s+([a-zA-Z]+)\\s*$";
    int begin_comp;
    begin_comp = regcomp(&begin_regex, begin_regex_string, REG_EXTENDED);
    ...
    if(begin_comp){
      ...
      exec_error = regexec(&begin_regex, full_command, 4, create_matches, 0);
      ...
      if(!exec_error){
        ...
        if(strlen(user_create_arg) > MAX_USERNAME){
          ...
          if (card_file != NULL){
            ...
            //check that bank has record of this user securely
    ...
    char* withdraw_regex_string = "^\\s*withdraw\\s+([0-9]+)\\s*$";
    int with_comp;
    ...
    if(with_comp){
      ...
      if(!exec_error){
        ...
        //extracting the argument username
        strncpy(amount_arg, &full_command[start], end - start);
